#!/bin/bash
# get-release.sh - Download latest release JAR and Docker image
...
get_release() {
    ...
    JAR_RELEASE=$(echo "$RELEASES_INFO" | tr '\000-\037' ' ' | jq -r '.[] | select(.draft == false and .prerelease == false and (.assets[] | .name | endswith(".jar"))) | @base64' | head -1)
    ...
    DOCKER_RELEASE=$(echo "$RELEASES_INFO" | tr '\000-\037' ' ' | jq -r '.[] | select(.draft == false and .prerelease == false and (.assets[] | .name | test("-docker\\.tar(\\.gz)?$"))) | @base64' | head -1)
